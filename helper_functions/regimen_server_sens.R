regimen_server_sens <- function(id) {
  moduleServer(id, function(input, output, session) {
    ns <- session$ns
    # Change from list to a single regimen
    regimen <- reactiveVal(NULL)
    ...
    observeEvent(input$add_regimen, {
      showModal(modalDialog(
        ...
        selectInput(ns("dosing_strategy"), "Dosing Strategy",
                    choices = c("Allometric Customized WB-based" = "costum_allometric_WB")),
    ...
    observeEvent(input$save_regimen, {
      ...
      # Store a single regimen instead of appending to a list
      regimen(new_regimen)
      removeModal()
    ...
    output$saved_regimens <- renderUI({
      if (is.null(regimen())) return(NULL)
      
      # Display the single saved regimen
      div(
        h4(class = "mb-3 text-primary", "Saved Regimen"),  # Section title
        div(
    ...
    # Delete the regimen
    observeEvent(input$delete_regimen, {
      regimen(NULL)  # Set to NULL to indicate no regimen exists
    })
    
    
    return(list(
      regimen = regimen,
      get_regimen = reactive({
        regimen()  # Return the single regimen
      })
    ))
  })
}